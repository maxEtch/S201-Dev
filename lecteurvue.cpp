...
void LecteurVue::lancer()
{
    ...
    timer->start(_vitesseDefilement); //on lance le chronomètre
}
...
void LecteurVue::modifVitesse()
{
    ...
    v.exec();
    _vitesseDefilement = v.getVitesse() * 1000;

    //modifie la vitesse dans la base de données
    QSqlQuery query;
    query.prepare("UPDATE Diaporamas SET `vitesseDefilement` = :nouvelleVitesse WHERE idDiaporama = :numDiaporamaCourant; ");

    query.bindValue(":nouvelleVitesse", v.getVitesse());
    query.bindValue(":numDiaporamaCourant",  _numDiaporamaCourant);
...
{
    _vitesseDefilement = nouvelleVitesse;
}
