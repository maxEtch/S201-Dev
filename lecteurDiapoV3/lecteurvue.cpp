...
void LecteurVue::creerBarStatus(bool estAutomatique)   //change l'état de la barre de status
{
    if (estAutomatique)
    {
...
void LecteurVue::lancer() //marche
{
    ...
    automatique = true;
    this->creerBarStatus(automatique);           //Change la bar de status pour afficher le mode automatique
}
...
void LecteurVue::arreter() //marche
{
    ...
    automatique = false;
    this->creerBarStatus(automatique);            //Change l'état de la bar de status pour afficher le mode manuel
}
...
{
    QMessageBox::information(this, "À propos de", "Version : Version 3.0\n Date création : 09/05/2023\n Auteurs : Maxime Etchecopar, Axel Marrier, Lucas Marrot"); //description provisoir
...
{
    qDebug() << "le bouton enlever diaporama marche";
}
